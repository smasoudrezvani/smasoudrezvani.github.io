...
In machine learning, we spend most of our time optimizing for pure prediction. We feed thousands of features into complex models to minimize a loss function. But when it comes to business strategy, public policy, or algorithmic trading, predicting an outcome isn't enough; we need to know what _caused_ it.
...
$$ (Y*{it} - \overline{Y}\_i) = \beta_1(X*{it} - \overline{X}_i) + (u_{it} - \overline{u}\_i) $$

The unobserved fixed effect $$\alpha_i$$ is eliminated because $$\alpha_i - \alpha_i = 0$$.

> ##### KEY ASSUMPTIONS
>
> **Strict Exogeneity** ($$E[u_{it} | X_{i1},...,X_{iT}, \alpha_i] = 0$$), which explicitly rules out feedback loops where today's shock affects tomorrow's treatment. _Note: You must use clustered standard errors to account for serial correlation within entities._
> {: .block-warning }
...
_Interact with the chart below to see how DiD isolates the treatment effect from the counterfactual trend:_
...
$$ Y*{it} = \beta_0 + \beta_1 X*{it} + \beta*2 D_i + \beta_3 S_t + u*{it} $$
